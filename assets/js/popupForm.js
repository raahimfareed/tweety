$(function() {
    $(document).on('click', '.addTweetBtn', function(){
        ...
        $.post('http://localhost:1111/tweety/core/ajax/tweetForm.php', function(data) {
            $('.popupTweet').html(data);
            $('.closeTweetPopup').click(function() {
                $('.popup-tweet-wrap').hide();
                $('.status-removed').removeClass().addClass('status');
                $('.hash-removed').removeClass().addClass('hash');
    ...
    $(document).on('submit', '#popupForm', function(e) {
        ...
        $.ajax({
            url: "http://localhost:1111/tweety/core/ajax/addTweet.php",
            type: "POST",
            data: formData,
